document.addEventListener('DOMContentLoaded', function () {
    ...
    function handleRegistration(event) {
        ...
        fetch('http://localhost:3000/api/register', { // Używaj pełnej ścieżki URL
            method: 'POST',
            headers: {
    ...
    function handleLogin(event) {
        ...
        fetch('http://localhost:3000/api/login', { // Używaj pełnej ścieżki URL
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    function loadUserProfilePicture() {
        ...
        if (token) {
            fetch('http://localhost:3000/api/profile', { // Używaj pełnej ścieżki URL
                method: 'GET',
                headers: {
                ...
                .then(response => {
                    if (!response.ok) {
                        if (response.status === 401 || response.status === 403) {
                            localStorage.removeItem('token')
                            window.location.href = 'index.html' // Używaj pełnej ścieżki URL
                        }
