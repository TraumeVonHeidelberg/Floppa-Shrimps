document.addEventListener('DOMContentLoaded', function () {
	...
	function deleteItem(type, id) {
		const url =
			type === 'menu' ? `${API_URL}/menu/${id}` :
			type === 'testimonial' ? `${API_URL}/testimonials/${id}` :
		...
		.catch(error => {
			console.error('Error:', error);
			alert(`Wystąpił błąd podczas usuwania ${type === 'menu' ? 'pozycji' : type === 'testimonial' ? 'testimonialu' : 'rezerwacji'}.`);
		});
	}
